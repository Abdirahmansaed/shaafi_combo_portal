    ...
    <div class="max-w-6xl space-y-5">
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5">
            ...
            <form method="POST" action="{{ route('settings.users.store') }}" class="grid gap-4 sm:grid-cols-2">
                ...
                <label class="text-sm"><span class="mb-1 block text-slate-600">Account Status</span><select name="status" required class="w-full rounded-md border border-slate-300 p-2"><option value="ACTIVE" @selected(old('status', 'ACTIVE') === 'ACTIVE')>Active</option><option value="INIT" @selected(old('status') === 'INIT')>Pending activation</option></select><x-auth-error field="status" /></label>
        ...
        <section class="overflow-hidden rounded-lg border border-slate-200 bg-white">
            ...
            @php($statusOptions = ['ACTIVE' => 'ACTIVE', 'INIT' => 'PENDING', 'DELETED' => 'DEACTIVATED'])
            <div class="overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="bg-slate-50 text-slate-500"><tr><th class="px-5 py-3 font-medium">Name</th><th class="px-5 py-3 font-medium">Username</th><th class="px-5 py-3 font-medium">Role</th><th class="px-5 py-3 font-medium">Account Status</th><th class="px-5 py-3 font-medium">Manage Account</th></tr></thead><tbody class="divide-y divide-slate-100">@foreach($users as $user)<tr><td class="px-5 py-3"><p class="font-medium">{{ $user->displayName() }}</p><p class="text-xs text-slate-500">{{ $user->number }}</p></td><td class="px-5 py-3">{{ $user->username }}</td><td class="px-5 py-3">{{ $user->roleLabel() }}</td><td class="px-5 py-3">{{ $user->statusLabel() }}</td><td class="px-5 py-3">@if($user->isSuperAdmin()) <span class="text-xs text-slate-500">Superadmin account</span> @else <form method="POST" action="{{ route('settings.users.status', $user) }}" class="flex gap-2">@csrf @method('PATCH')<select name="status" aria-label="Account status for {{ $user->displayName() }}" class="rounded border border-slate-300 px-2 py-1 text-xs">@foreach($statusOptions as $value => $label)<option value="{{ $value }}" {{ $user->status === $value ? 'selected' : '' }}>{{ $label }}</option>@endforeach</select><button class="text-xs font-medium text-teal-700">Update status</button></form> @endif</td></tr>@endforeach</tbody></table></div>
        </section>
    </div>
</x-layouts.app>
