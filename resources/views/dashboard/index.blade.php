...
<div class="mb-6 flex items-end justify-between"><p class="text-sm text-slate-500">An overview of Combo Package activity and subscriptions. <span class="ml-2 inline-flex items-center gap-1"><label for="dashboard-date-mode" class="font-medium text-slate-700">Date:</label><select id="dashboard-date-mode" class="rounded border-slate-300 py-0 text-sm"><option value="all" @selected(! $selectedDate)>All</option><option value="specific" @selected($selectedDate)>Specific Date</option></select><input id="dashboard-date" type="date" value="{{ $selectedDate?->toDateString() }}" class="rounded border-slate-300 py-0 text-sm @if(! $selectedDate) hidden @endif"><button id="dashboard-date-reset" type="button" class="text-teal-700 @if(! $selectedDate) hidden @endif">Reset</button></span></p><p id="dashboard-last-updated" class="text-xs text-slate-400">Last updated: {{ now()->format('d M Y, H:i:s') }}</p></div>
