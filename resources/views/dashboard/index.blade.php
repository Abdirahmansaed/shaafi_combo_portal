...
<div class="grid grid-cols-2 gap-3 lg:grid-cols-4 xl:grid-cols-7">@foreach($stats as $label=>$value)<x-stat-card :label="$label" :value="$value" :live-key="$label" :icon="str_contains($label,'Daily')?'sun':(str_contains($label,'Weekly')?'calendar-days':(str_contains($label,'Monthly')?'calendar':'users'))"/>@endforeach</div>
...
(() => {
    ...
    async function refreshDashboard() {
        ...
        inFlight = true;
        try {
            const response = await fetch('{{ route('dashboard.live') }}', {headers: {'Accept': 'application/json'}, credentials: 'same-origin'});
            ...
            lastUpdated.textContent = 'Live update temporarily unavailable';
